...
install_info="${common_parse_dir}/share/info/metadef/ascend_install.info"
...
log() {
    ...
    if [ "$log_type" = "INFO" -o "$log_type" = "WARNING" -o "$log_type" = "ERROR" ]; then
        echo -e "[metadef] [$cur_date] [$log_type]: $*"
    else
        echo "[metadef] [$cur_date] [$log_type]: $*" 1> /dev/null
    fi
    echo "[metadef] [$cur_date] [$log_type]: $*" >> "$logfile"
}
...
new_upgrade() {
    if [ ! -d "${sourcedir}" ]; then
        log "INFO" "no need to upgrade metadef files."
        return 0
    fi
    output_progress 10
    ...
    sh "$curpath/install_common_parser.sh" --package="metadef" --install --username="$username" --usergroup="$usergroup" --set-cann-uninstall --upgrade \
