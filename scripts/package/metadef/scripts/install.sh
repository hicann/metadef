...
log() {
    ...
    local log_type="$1"
    local log_msg="$2"
    local log_format="[metadef] [$cur_date] [$log_type]: $log_msg"
...
update_install_param() {
    ...
    local install_info_key_array="METADEF_Install_Type METADEF_UserName METADEF_UserGroup METADEF_Install_Path_Param"
...
update_install_info() {
    ...
    update_install_param "METADEF_Install_Type" "$installmode" "$install_info"
    update_install_param "METADEF_UserName" "$username" "$install_info"
...
install_run() {
    ...
    local metadef_install_path_param=""
    update_install_path
    update_install_info
    local metadef_input_install_path=$(get_install_param "METADEF_Install_Path_Param" "${install_info}")
    local metadef_install_type=$(get_install_param "METADEF_Install_Type" "${install_info}")
    if [ "$is_docker_install" = "y" ]; then
        metadef_install_path_param=$(concat_docker_install_path "${docker_root}" "${metadef_input_install_path}")
    else
        metadef_install_path_param="${metadef_input_install_path}"
    fi
    if [ "$1" = "install" ]; then
        unchattr_files
        chmod_start
        new_echo "INFO" "install ${metadef_install_path_param} ${metadef_install_type}"
        log "INFO" "install ${metadef_install_path_param} ${metadef_install_type}"
        bash "${curpath}/run_metadef_install.sh" "install" "${metadef_input_install_path}" "${metadef_install_type}" \
            "${is_quiet}" "${pylocal}" "${input_setenv}" "${docker_root}" "${in_install_for_all}" "$pkg_version_dir"
        if [ $? -eq 0 ]; then
            update_version_info_version
            log "INFO" "metadef package installed successfully! The new version takes effect immediately."
            log_operation "${operation}" "succeeded"
            chmod_end
            prompt_set_env "${install_path_param}"
        else
            chmod_end
            log "ERROR" "metadef package install failed, please retry after uninstall!"
...
upgrade_run() {
    ...
    if [ -f "$install_info" ]; then
        local metadef_input_install_path=$(get_install_param "METADEF_Install_Path_Param" "${install_info}")
        local metadef_install_type=$(get_install_param "METADEF_Install_Type" "${install_info}")
    elif [ -f "$install_info_old" ] && [ $(grep -c -i metadef_install_path_param "$install_info_old") -ne 0 ]; then
        local metadef_input_install_path="$(grep -iw metadef_install_path_param "$install_info_old" | cut -d"=" -f2-)"
        local metadef_install_type="$(grep -iw metadef_install_type "$install_info_old" | cut -d"=" -f2-)"
    else
    ...
    if [ "$1" = "upgrade" ]; then
        chmod_start
        new_echo "INFO" "upgrade ${metadef_install_path_param} ${metadef_install_type}"
        log "INFO" "upgrade ${metadef_install_path_param} ${metadef_install_type}"
        bash "${curpath}/run_metadef_upgrade.sh" "upgrade" "${metadef_input_install_path}" "${metadef_install_type}" \
            "${is_quiet}" "${pylocal}" "${input_setenv}" "${docker_root}" "${in_install_for_all}" "$pkg_version_dir"
        if [ $? -eq 0 ]; then
            update_version_info_version
            log "INFO" "metadef package upgraded successfully! The new version takes effect immediately."
            log_operation "${operation}" "succeeded"
            chmod_end
            prompt_set_env "${install_path_param}"
        else
            chmod_end
            log "ERROR" "metadef package upgrade failed, please retry after uninstall!"
            log_operation "${operation}" "failed!"
            exit_install_log 1
        fi
    fi
...
uninstall_run() {
    ...
    if [ -f "$upgrade_install_info" ]; then
        local metadef_input_install_path=$(get_install_param "METADEF_Install_Path_Param" "${upgrade_install_info}")
        local metadef_install_type=$(get_install_param "METADEF_Install_Type" "${upgrade_install_info}")
    elif [ -f "$install_info_old" ] && [ $(grep -c -i metadef_install_path_param "$install_info_old") -ne 0 ]; then
        local metadef_input_install_path="$(grep -iw metadef_install_path_param "$install_info_old" | cut -d"=" -f2-)"
        local metadef_install_type="$(grep -iw metadef_install_type "$install_info_old" | cut -d"=" -f2-)"
    else
    ...
    if [ ! -f "$upgrade_default_dir/script/run_metadef_uninstall.sh" ]; then
        log "WARNING" "run_metadef_uninstall.sh not found."
        return $?
    fi
    if [ "$1" = "uninstall" ]; then
        chmod_start "$upgrade_default_dir"
        new_echo "INFO" "uninstall ${metadef_install_path_param} ${metadef_install_type}"
        log "INFO" "uninstall ${metadef_install_path_param} ${metadef_install_type}"
        bash "$upgrade_default_dir/script/run_metadef_uninstall.sh" "uninstall" "${metadef_input_install_path}" "${metadef_install_type}" "${is_quiet}" \
        ...
        if [ $? -eq 0 ]; then
            ...
            if [ $(id -u) -eq 0 ]; then
                if [ "$uninstall" = "y" ] && [ -f "$install_info_old" ] && [ $(grep -c -i metadef_install_path_param "$install_info_old") -ne 0 ]; then
                    sed -i '/metadef_install_path_param=/Id' "$install_info_old" 2> /dev/null
                    sed -i '/metadef_install_type=/Id' "$install_info_old" 2> /dev/null
                fi
            fi
            new_echo "INFO" "metadef package uninstalled successfully! Uninstallation takes effect immediately."
            log "INFO" "metadef package uninstalled successfully! Uninstallation takes effect immediately."
            log_operation "${operation}" "succeeded"
        else
            log "ERROR" "metadef package uninstall failed!"
            log_operation "${operation}" "failed!"
            exit_uninstall_log 1
        fi
    fi
...
judgmentpath() {
    ...
    if [ $? -ne 0 ]; then
        log "ERROR" "The metadef install_path ${1} is invalid, only characters in [a-z,A-Z,0-9,-,_] are supported!"
        exit 1
    fi
}
...
migrate_user_assets_v2() {
    if [ "$pkg_is_multi_version" = "true" ]; then
        ...
        if [ -n "$last_installed_dir" ]; then
            ...
            local data_dir="metadef/data/fusion_strategy"
...
if [ "$check" = "y" ]; then
    ver_check
    preinstall_check --install-path="$pkg_install_path/$pkg_version_dir" --script-dir="$curpath" --package="metadef" --logfile="$logfile" --docker-root="$docker_root"
    if [ $? -ne 0 ]; then
        exit_install_log 1
    else
    ...
    ver_check
    preinstall_process --install-path="$pkg_install_path/$pkg_version_dir" --script-dir="$curpath" --package="metadef" --logfile="$logfile" --docker-root="$docker_root"
    if [ $? -ne 0 ]; then
        exit_install_log 1
    else
        log "INFO" "version compatibility check successfully!"
...
if [ "x$version_installed" != "x" -a "$version_installed" != "none" ] || [ -f "${install_info}" ]; then
    ...
    # 安装场景
        ...
        if [ "$is_quiet" = "n" ]; then
            log "INFO" "metadef package has been installed on the path ${pkg_install_path}, the version is ${version_installed}, and the version of this package is ${version_of_package}, do you want to continue? [y/n]"
            while true
            do
                read yn
                if test "$yn" = n; then
