...
new_install() {
    if [ ! -d "${sourcedir}" ]; then
        log "INFO" "no need to install metadef files."
        return 0
    fi
    output_progress 10
    ...
    sh "$curpath/install_common_parser.sh" --package="metadef" --install --username="$username" --usergroup="$usergroup" --set-cann-uninstall \
