...
void *OpExecuteContext::MallocWorkspace(const size_t size) {
  auto memory_vec =
      GetOutputPointer<std::vector<GertMemBlock *>>(static_cast<size_t>(OpExecuteOutputIndex::kBlockMemory));
  if (memory_vec == nullptr) {
    return nullptr;
  }
  const size_t input_num = GetComputeNodeInputNum();
  const size_t output_num = GetComputeNodeOutputNum();
  auto gert_allocator = GetInputValue<GertAllocator *>(input_num + output_num +
...
void OpExecuteContext::FreeWorkspace() {
  auto memory_vec =
      GetOutputPointer<std::vector<GertMemBlock *>>(static_cast<size_t>(OpExecuteOutputIndex::kBlockMemory));
  auto gert_allocator = GetInputValue<GertAllocator *>(GetComputeNodeInputNum() + GetComputeNodeOutputNum() +
                                                       static_cast<size_t>(OpExecuteInputExtendIndex::kAllocate));
