...
ge::graphStatus OmOpImplRegistryHolder::CreateOmOppDir(std::string &opp_dir) const {
  ...
  if (opp_dir.empty()) {
    ...
    if ((path_env == nullptr) || (strnlen(path_env, static_cast<size_t>(MMPA_MAX_PATH)) == 0U)) {
      GELOGE(ge::FAILED, "Get HOME path failed.");
...
ge::graphStatus OpImplRegistryHolder::GetOpImplFunctionsByHandle(const void *handle, const std::string &so_path) {
  for (size_t i = 0; i < static_cast<size_t>(ImplType::END_TYPE); ++i) {
    ...
    const auto get_impl_num =
        reinterpret_cast<GetImplNum>(mmDlsym(const_cast<void *>(handle), impl_menu.get_reg_num_func.c_str()));
...
void OpImplRegistryHolderManager::AddRegistry(const std::string &so_data,
...
const std::shared_ptr<OpImplRegistryHolder> OpImplRegistryHolderManager::GetOpImplRegistryHolder(
...
OpImplRegistryHolderPtr OpImplRegistryHolderManager::GetOrCreateOpImplRegistryHolder(
  ...
  if (create_func == nullptr) {
    GELOGW("create_func is nullptr");
    return nullptr;
  }
  auto registry_holder = create_func();
  if (registry_holder == nullptr) {
    GELOGW("create registry holder failed");
    return nullptr;
  }
  op_impl_registries_[so_data] = registry_holder;
