...
description: |
  metadef 仓库本地 pre-commit 检查。在创建 PR 或推送代码前，必须运行本地 pre-commit 检查，确保 codespell、clang-format、end-of-file-fixer 等 hooks 全部通过，避免 CI `codecheck_precommit` 任务重复失败。
...
# metadef 仓库 pre-commit 检查

## 背景

metadef 仓库的 CI 流水线开启了 `codecheck_precommit` 任务，会对 PR 中改动的文件运行 pre-commit hooks（codespell、clang-format、end-of-file-fixer 等）。如果本地不提前检查，CI 会重复失败，浪费流水线资源。
