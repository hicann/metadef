...
![](../../figures/opinfershape_context_builder.png)

使用步骤如下：

1. 构造ContextHolder

   调用OpInferShapeRangeContextBuilder接口，传入相应的输入输出数据，比如输入Tensor的Shape范围`gert::Range<gert::Tensor>`和输出的TensorDesc描述等，最终调用Build\(\)接口构造一个`ContextHolder<InferShapeRangeContext>`对象。
