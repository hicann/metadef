...
#### 安装依赖
  ...
  以下所列为源码编译用到的依赖，请注意版本要求（如果遇到安装问题，请尝试切换镜像源）。
  > [!NOTE] 注意
...
若您的编译环境可以访问网络，或已经完成[离线编译指导](./user_guides/offline_compile.md)，`metadef`提供一键式编译能力，可通过如下命令进行编译：
...
执行本地验证前，请确保已完成[安装依赖](#42-安装依赖)，并设置[环境变量](#3-环境变量配置)。
