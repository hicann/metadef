...
| --- | --- | --- | --- |
| Graph运行时相关接口 | exe_graph/runtime/tiling_context.h | Tiling函数输入、输出、属性及平台信息上下文。 | Header-only上下文；主要消费库 libmetadef.so/libexe_graph.so |
...
| Graph、GE与公共类型类接口 | graph/tensor.h | Shape、TensorDesc、Tensor公共接口。 | libgraph_base.so（实现位于GE/Air） |
...
| 注册内部接口与基础支撑类接口 | pkg_inc/base/registry/op_impl_space_registry_v2.h | 算子实现空间的创建、合并和查询。 | libopp_registry.so |
...
| Context Builder与Runtime定义类接口 | base/context_builder/context_holder.h | Kernel Context内存所有权和生命周期管理。 | libmetadef.so |
...
| 算子注册类接口 | register/op_impl_registry.h | Host侧算子Tiling、推导和执行函数注册。 | libopp_registry.so（同时编入librt2_registry.a） |
