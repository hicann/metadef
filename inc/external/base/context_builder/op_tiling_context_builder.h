...
class OpTilingContextBuilder : public OpContextBuilderBase<OpTilingContextBuilder> {
 public:
   ...
   * @note
   * 设置的所有输入数据类型，所有权归调用者管理，调用者需要保证输入指针生命周期指针长于Build产生的ContextHolder对象
   * @param deterministic_level 确定性计算级别，当前只支持三种级别，0：未开启任何配置。1：仅开启确定性计算配置。2:
   * 开启确定性计算和 强一致性计算配置。
   * @return TilingContextBuilder对象用于链式调用
   */
...
gert_TilingContextBuilder_SetDeterministicLevel(void *builder, int32_t deterministic_level);
