...
namespace ge {
int ge_log_level = ge::GeLogLevel::kDlogError;
 ...
 public:
  DefaultSlogStub() : SlogStub() {
    ...
    if (log_level != nullptr) {
      ...
      SetLevel(ge::GeLogLevel::kDlogError);
    }
  ...
  void Log(int module_id, int level, const char *fmt, va_list args) override {
    ...
    if (level == ge::GeLogLevel::kDlogEvent && GetEventLevel() < 1) {
      return;
    }
...
SlogStub::~SlogStub() {
  log_init = false;
  ge_log_level = ge::GeLogLevel::kDlogError;
}
...
void DlogRecord(int moduleId, int level, const char *fmt, ...) {
  ...
  if (moduleId & RUN_LOG_MASK) {
    ge::SlogStub::GetInstance()->Log(moduleId & (~RUN_LOG_MASK), ge::GeLogLevel::kDlogEvent, fmt, valist);
  } else {
...
void DlogErrorInner(int module_id, const char *fmt, ...) {
  va_list valist;
  va_start(valist, fmt);
  ge::SlogStub::GetInstance()->Log(module_id, ge::GeLogLevel::kDlogError, fmt, valist);
  va_end(valist);
}
...
void DlogWarnInner(int module_id, const char *fmt, ...) {
  va_list valist;
  va_start(valist, fmt);
  ge::SlogStub::GetInstance()->Log(module_id, ge::GeLogLevel::kDlogWarn, fmt, valist);
  va_end(valist);
}
...
void DlogInfoInner(int module_id, const char *fmt, ...) {
  ...
  if (module_id & RUN_LOG_MASK) {
    ge::SlogStub::GetInstance()->Log(module_id & (~RUN_LOG_MASK), ge::GeLogLevel::kDlogEvent, fmt, valist);
  } else {
    ge::SlogStub::GetInstance()->Log(module_id, ge::GeLogLevel::kDlogInfo, fmt, valist);
  }
  va_end(valist);
}
...
void DlogDebugInner(int module_id, const char *fmt, ...) {
  va_list valist;
  va_start(valist, fmt);
  ge::SlogStub::GetInstance()->Log(module_id, ge::GeLogLevel::kDlogDebug, fmt, valist);
  va_end(valist);
}
