...
"""
    ...
    when DEBUG on
"""
white_list_for_debug = ["error_manager.h", "err_msg.h"]
include_dir_key_words = ["error_manager", "base"]
...
class H2CC(object):
    ...
    def extract_param_name(param_str):
        ...
        # 查找参数名
        # 从末尾开始查找，跳过空格、引用、指针和const等修饰符
        # 正则表达式匹配：找到最后一个标识符，它前面可能有&、*或&&
        # 但要注意：const std::string &root_graph_name 中的 & 应该在标识符前面
        # 我们需要找到最后一个单词，它可能是参数名
        ...
        for i in range(len(parts) - 1, -1, -1):
            part = parts[i]
            # 检查是否是C++类型修饰符
            if part in ['const', 'volatile', 'static', 'register', 'extern',
    ...
    @staticmethod
    def extract_param_names(func_decl):
        """从C++函数声明中提取参数名"""
        # 首先去除可能的换行和多余空格
        func_decl = ' '.join(func_decl.split())

        # 查找参数列表部分
        # 移除类名和作用域解析符
        match = re.search(r'(\w+::)?\w+\s*\((.*?)\)(?:\s*const)?\s*;?$', func_decl)
        if not match:
            # 尝试匹配没有类作用域的情况
            match = re.search(r'\w+\s*\((.*?)\)(?:\s*const)?\s*;?$', func_decl)
            if not match:
                return ""
        ...
        for i, ch in enumerate(params):
            if ch == '(':
                ...
                param = params[start:i].strip()
                # 调用另一个静态方法
                param_name = H2CC.extract_param_name(param)
                if param_name:
        ...
        # 处理最后一个参数
        if start < len(params):
            param = params[start:].strip()
            param_name = H2CC.extract_param_name(param)
            if param_name:
                param_names.append(param_name)

        # 过滤空值并生成结果
        return '\n'.join(f'  (void){name};' for name in param_names if name)

    @staticmethod
    def implement_function(func):
        function_def = ''
        function_def += '{\n'
        void_lines = H2CC.extract_param_names(func)
        if void_lines is not None:
            function_def += void_lines
            function_def += '\n'
