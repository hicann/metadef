...
int32_t ErrorManager::ParseJsonFile(const std::string path) {
  ...
  return ParseJsonFormatString(ge::PtrToPtr<nlohmann::json, void>(&json_file));
}
///
...
/// @return int 0(success) -1(fail)
///
int32_t ErrorManager::ParseJsonFormatString(const void *const handle, int32_t priority) {
  GELOGD("Begin to parse json string");
  try {
    const nlohmann::json *const json_file = ge::PtrToPtr<void, nlohmann::json>(handle);
    ...
    for (const auto &error_json : error_list_json) {
      ...
      if (it == error_map_.cend()) {
        ...
        if (it->second.priority < error_info.priority) {
          ...
          GELOGD("Update error_code %s success, current priority[%d] is greater than saved priority[%d]",
                 error_info.error_id.c_str(), priority, it->second.priority);
        } else {
          GELOGD("No need update error_code %s, due to current priority[%d] is less and equal than saved priority[%d]",
                 error_info.error_id.c_str(), priority, it->second.priority);
        }
      }
    }
...
int32_t RegisterFormatErrorMessage(const char_t *error_msg, size_t error_msg_len) {
  ...
  // 用户注册的error code的优先级高于配置文件的, 默认优先级为0，此处priority使用1
  return ErrorManager::GetInstance().ParseJsonFormatString(ge::PtrToPtr<nlohmann::json, void>(&j), 1);
}
